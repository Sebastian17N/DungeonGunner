...
public static class HelperUtilities
{
    ...
    public static Vector3 GetMouseWorldPosition()
    {
        ...
        Vector3 worldPosition = mainCamera.ScreenToWorldPoint(mouseScreenPositon);
        worldPosition.z = 0f;
    ...
    public static bool ValidateCheckPositiveValue(Object thisObject, string fieldName, int valueToCheck, bool isZeroAllowed)
    {
        ...
        else
        {
            if (valueToCheck <= 0)
            {
                Debug.Log(fieldName + " must contain a positive value" + thisObject.name.ToString());
                error = true;
            }
        }
    ...
    public static Vector3 GetSpawnPositionNearestToPlayer(Vector3 playerPosition)
    {
        ...
        Vector3 nearestSpawnPositions = new Vector3(10000f, 10000f, 0f);

        foreach (Vector2Int spawnPositionGrid in currentRoom.spawnPositionsArray)
        {
            Vector3 spawnPositionWorld = grid.CellToWorld((Vector3Int)spawnPositionGrid);

            if (Vector3.Distance(spawnPositionWorld, playerPosition) < Vector3.Distance(nearestSpawnPositions, playerPosition))
            {
                nearestSpawnPositions = spawnPositionWorld;
            }
        }

        return nearestSpawnPositions;
    }
}
